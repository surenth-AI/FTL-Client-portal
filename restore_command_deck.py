...
{% block content %}
<!-- Leaflet Engine Integration for Dynamic Visual Inputs -->
...
<div class="intel-fade-in" style="animation-delay: 0.1s;">
...
<script>
    function addWizLCLRow() {
        if(typeof lclItemCounter === 'undefined') window.lclItemCounter = 0;
        window.lclItemCounter++;
        const id = `wiz-lcl-row-${window.lclItemCounter}`;
        const div = document.createElement('div');
        div.className = 'cargo-hologram';
        div.id = id;
        div.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-2">
                <span style="font-size: 0.7rem; font-weight: 700; color: var(--neon-cyan); letter-spacing: 1px;">UNIT #${window.lclItemCounter}</span>
                ${window.lclItemCounter > 1 ? `<span style="cursor:pointer; font-size:0.8rem; color: rgba(255,255,255,0.4);" onclick="document.getElementById('${id}').remove()">✕</span>` : ''}
            </div>
            <div class="holo-grid">
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">QTY</label>
                    <input type="number" name="item_qty[]" class="cyber-input" value="1" min="1" style="padding: 0.4rem;">
                </div>
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">TYPE</label>
                    <select name="item_type[]" class="cyber-input" style="padding: 0.4rem; color: #000;">
                        <option value="Pallet">Pallet</option><option value="Carton">Carton</option>
                    </select>
                </div>
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">WT (KG)</label>
                    <input type="number" name="item_weight[]" class="cyber-input" placeholder="0.0" step="0.01" style="padding: 0.4rem;">
                </div>
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">VOL (CBM)</label>
                    <input type="number" name="item_volume[]" class="cyber-input" placeholder="0.00" step="0.01" style="padding: 0.4rem;">
                </div>
            </div>
            <div class="mt-2 d-flex align-items-center gap-2">
                <label class="cyber-switch">
                    <input type="checkbox" name="item_stackable[]" value="yes" checked>
                    <span class="slider"></span>
                </label>
                <span style="font-size: 0.65rem; color: var(--text-dim);">Stackable</span>
                
                <label class="cyber-switch ms-2">
                    <input type="checkbox" onchange="document.getElementById('${id}-hz').style.display = this.checked ? 'block' : 'none'; document.getElementById('${id}-imo-hidden').value = this.checked ? 'yes' : 'no';">
                    <span class="slider" style="border-color: #f43f5e;"></span>
                </label>
                <span style="font-size: 0.65rem; color: #f43f5e;">Hazardous</span>
                <input type="hidden" name="item_is_imo[]" id="${id}-imo-hidden" value="no">
            </div>
            
            <div id="${id}-hz" style="display:none; margin-top:0.5rem; padding-top:0.5rem; border-top:1px dashed rgba(244, 63, 94, 0.3);">
                <div class="holo-grid">
                    <div><input type="text" name="item_un[]" class="cyber-input" placeholder="UN" style="padding:0.3rem; font-size:0.7rem;"></div>
                    <div><input type="text" name="item_class[]" class="cyber-input" placeholder="Class" style="padding:0.3rem; font-size:0.7rem;"></div>
                    <div><input type="text" name="item_pg[]" class="cyber-input" placeholder="PG" style="padding:0.3rem; font-size:0.7rem;"></div>
                </div>
            </div>
        `;
        document.getElementById('wiz_lcl_rows').appendChild(div);
    }

    function addWizFCLRow() {
        if(typeof fclItemCounter === 'undefined') window.fclItemCounter = 0;
        window.fclItemCounter++;
        const id = `wiz-fcl-row-${window.fclItemCounter}`;
        const div = document.createElement('div');
        div.className = 'cargo-hologram';
        div.id = id;
        div.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-2">
                <span style="font-size: 0.7rem; font-weight: 700; color: var(--neon-indigo); letter-spacing: 1px;">CONT #${window.fclItemCounter}</span>
                ${window.fclItemCounter > 1 ? `<span style="cursor:pointer; font-size:0.8rem; color: rgba(255,255,255,0.4);" onclick="document.getElementById('${id}').remove()">✕</span>` : ''}
            </div>
            <div class="holo-grid">
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">QTY</label>
                    <input type="number" name="cont_qty[]" class="cyber-input" value="1" min="1" style="padding: 0.4rem;">
                </div>
                <div style="grid-column: span 2;">
                    <label class="cyber-label" style="font-size:0.55rem;">TYPE</label>
                    <select name="cont_type[]" class="cyber-input" style="padding: 0.4rem; color:#000;">
                        <option value="20' ST">20' Standard</option><option value="40' ST">40' Standard</option><option value="40' HC">40' High Cube</option>
                    </select>
                </div>
                <div>
                    <label class="cyber-label" style="font-size:0.55rem;">WT (KG)</label>
                    <input type="number" name="cont_weight[]" class="cyber-input" placeholder="0.0" step="0.01" style="padding: 0.4rem;">
                </div>
            </div>
        `;
        document.getElementById('wiz_fcl_rows').appendChild(div);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (document.getElementById('globalRouteMap')) {
            globalRouteMap = L.map('globalRouteMap', {
    ...
    function setWizService(type) {
        document.getElementById('wiz_hidden_service_type').value = type;
        document.getElementById('tab_lcl').classList.remove('active');
        document.getElementById('tab_fcl').classList.remove('active');
        
        if (type === 'LCL') {
            document.getElementById('tab_lcl').classList.add('active');
            document.getElementById('wiz_lcl_section').style.display = 'block';
            document.getElementById('wiz_fcl_section').style.display = 'none';
        } else {
            document.getElementById('tab_fcl').classList.add('active');
            document.getElementById('wiz_fcl_section').style.display = 'block';
            document.getElementById('wiz_lcl_section').style.display = 'none';
        }
    }
</script>
{% endblock %}
...
print("rates.html completely replaced with Command Deck UI")
